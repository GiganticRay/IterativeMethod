...
int main(){
	// got the basic gpu info to run our own kernel function
	int deviceId;
	int numberOfSMs;
	...
	s_axpby(sizeof(b)/sizeof(double), 2, b, 0, NULL, b);
	double result = s_norm(sizeof(b)/sizeof(double), b, 2.0);
	...
	double	*dA_values, *vecq_value, **Q;

	CHECK_CUDA( cudaMalloc((void**) &dA_rows, 		nz * sizeof(int)));
	CHECK_CUDA( cudaMalloc((void**) &dA_columns, 	nz * sizeof(int)));
	CHECK_CUDA( cudaMalloc((void**) &dA_values,		nz * sizeof(double)));
	CHECK_CUDA( cudaMalloc((void**) &vecq_value,	N  * sizeof(double)));
	Q = (double**) malloc(N * sizeof(double*));

	CHECK_CUDA( cudaMemcpy(dA_rows, 	I, 	nz 	* sizeof(int), 	cudaMemcpyHostToDevice));
	CHECK_CUDA( cudaMemcpy(dA_columns, 	J, 	nz 	* sizeof(int), 	cudaMemcpyHostToDevice));
	CHECK_CUDA( cudaMemcpy(dA_values, 	val,nz 	* sizeof(double),cudaMemcpyHostToDevice));
	CHECK_CUDA( cudaMemcpy(vecq_value,	b, 	N 	* sizeof(double),cudaMemcpyHostToDevice));
	...
	CHECK_CUSPARSE( cusparseCreateCoo(	&matA, M, N, nz, dA_rows, dA_columns, dA_values, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));
	CHECK_CUSPARSE( cusparseCreateDnVec(&vecq, N, vecq_value, CUDA_R_64F));
